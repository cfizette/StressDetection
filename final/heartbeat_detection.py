# -*- coding: utf-8 -*-
"""
Created on Wed Nov 29 10:22:37 2017

@author: Cody
Taken from paulvangent.com
"""
...
def calc_RR(dataset, fs):
    ...
    while (cnt < (len(peaklist)-1)):
        ...
        cnt += 1
    measures['RR_list'] = RR_list # in ms
...
def process(dataset, hrw, fs, plot = False): #Remember; hrw was the one-sided window size (we used 0.75) and fs was the sample rate (file is recorded at 100Hz)
    rolmean(dataset, hrw, fs)
    detect_peaks(dataset)
    calc_RR(dataset, fs)
    calc_bpm()
    if plot:
        plotter(dataset, "Heartbeat")